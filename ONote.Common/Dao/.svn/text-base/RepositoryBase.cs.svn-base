...
namespace ONote.Common.Dao
{
	
    public abstract class RepositoryBase<T> : HibernateDaoSupport, IRepositoryBase<T> where T : class
    {
        ...
        public virtual IQueryable<T> LoadAllWithPage(out long count, int pageIndex, int pageSize,string orderBy,string sort)
        {
            var result = Session.Query<T>();
        ...
        #region IRepositoryBase<T> 成员


        public void DeleteEntity(IList<T> entity)
        {
            foreach (object e in entity)
            {
